...
export async function generateSalesReportPDF(data: ReportData) {
  ...
  await import("jspdf-autotable"); // ✅ just import — no need to call it
  const doc = new JsPDF();
  ...
  if (
    ...
    if (data.filters.paymentMethod !== "all") {
      doc.text(
        `Payment Method: ${data.filters.paymentMethod.toUpperCase()}`,
        20,
        yPosition
      );
      yPosition += 6;
  ...
  for (let i = 1; i <= totalPages; i++) {
    ...
    doc.text(new Date().toLocaleString(), pageWidth - 20, pageHeight - 10, {
      align: "right",
    });
  }
