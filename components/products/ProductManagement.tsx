...
export function ProductManagement() {
  ...
  const handleProductSave = async (productData: any) => {
    try {
      ...
      toast({
        title: "Error",
        description: error instanceof Error ? error.message : `Failed to ${editingProduct ? "update" : "add"} product`,
        variant: "destructive",
      })
    }
  }
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-red-600">
              {products.filter((p) => p.quantityInStock <= p.minStockLevel).length}
            </div>
          </CardContent>
        </Card>
        <Card>
                ...
                {filteredProducts.map((product) => {
                  ...
                  return (
                    <TableRow key={product.id}>
                      ...
                      <TableCell className="font-medium">${product.sellingPrice.toFixed(2)}</TableCell>
                      <TableCell>
